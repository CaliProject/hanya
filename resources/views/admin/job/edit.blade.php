...
@section('admin.title','招贤纳士-编辑内容')

@section('breadcrumb')
    <li><a href="{{ url('manage/job') }}">招贤纳士</a></li>
    <li class="active"><span>编辑内容</span></li>
@stop
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-body">
                    <form action="{{ $job->editLink() }}" method="post" class="Form">
                        {{ csrf_field() }}
                        {{ method_field('PATCH') }}
                        <div class="summernote"></div>
                        <div class="form-group">
                            <button type="submit" class="confirm-button">确认修改</button>
                            <a href="{{ url('manage/job') }}" class="btn btn-danger btn-block">返回</a>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(function () {
        ...
        setTimeout(function () {
            $(".summernote").summernote('code', '{!! empty($job->body) ? addslashes(old('body')) : $job->body  !!}');
        },500);
        @endif
